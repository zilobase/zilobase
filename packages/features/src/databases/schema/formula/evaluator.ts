...
import { formatFormulaValue, getFormulaValueType } from "./formatters"
...
function evaluateIdentifier(
  ...
  const propertyValue = resolveDatabaseFormulaPropertyValue(name, context)
...
function callFormulaFunction(
  ...
  if (lowerName === "prop") {
    ...
    return resolveDatabaseFormulaPropertyValue(propertyName, context).value
  }
  ...
  return callEagerFormulaFunction(lowerName, args, context)
}
...
function callFormulaMethod(
  ...
  switch (lowerMethod) {
    ...
    case "date":
      return callEagerFormulaFunction("date", [objectValue], context)
    case "day":
      return notionDay(requireDate(objectValue), context.timezone)
    case "email":
    case "name":
    ...
    case "formatdate":
      return formatFormulaDate(objectValue, textValue(args[0]), context)
    case "formatnumber":
      return formatFormulaNumber(objectValue, args[0], args[1], context.locale)
    case "hour":
      return callEagerFormulaFunction("hour", [objectValue], context)
    case "join":
    ...
    case "minute":
      return callEagerFormulaFunction("minute", [objectValue], context)
    case "month":
      return callEagerFormulaFunction("month", [objectValue], context)
    case "replace":
    ...
    case "week":
      return isoWeek(requireDate(objectValue), context.timezone)
    case "year":
      return callEagerFormulaFunction("year", [objectValue], context)
    default:
      throw new Error(`Unknown formula method: .${method}()`)
  }
}

function resolveDatabaseFormulaPropertyValue(
  propertyName: string,
  context: DatabaseFormulaEvaluationContext
...
function normalizePropertyValue(
  value: DatabaseFormulaPropertyValue | undefined,
  type: string,
  row: DatabaseFormulaRow
): FormulaValue {
