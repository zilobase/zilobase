...
pub(crate) fn meeting_capture_recoverable_sessions(
        ...
        .flatten()
    {
        ...
        if let Ok(session) = serde_json::from_slice::<RecoverableMeetingCapture>(&bytes) {
            let expected = entry.path().join("meeting-audio.wav");
            if validate_meeting_id(&session.meeting_id).is_ok()
                && entry.file_name() == session.meeting_id.as_str()
                && Path::new(&session.audio_path) == expected
                && expected.is_file()
            {
...
pub(crate) fn meeting_capture_delete_local_file(
    ...
    if fs::symlink_metadata(&directory).is_ok_and(|metadata| metadata.file_type().is_symlink()) {
        return Err("Invalid meeting checkpoint directory".into());
    }
    if directory.exists() {
...
pub(crate) fn meeting_capture_open_local_file(
    ...
    if fs::symlink_metadata(audio_path.parent().unwrap_or(&audio_path))
        .is_ok_and(|metadata| metadata.file_type().is_symlink())
        || fs::symlink_metadata(&audio_path).is_ok_and(|metadata| metadata.file_type().is_symlink())
    {
        return Err("Invalid meeting checkpoint path".into());
    }
...
mod tests {
    use super::{
        meeting_capture_recoverable_sessions, validate_meeting_id, RecoverableMeetingCapture,
    };
    use crate::meetings::AppHandle;
    use std::{
        fs, io,
    ...
    #[test]
    fn recovery_rejects_checkpoint_paths_outside_its_meeting_directory() {
        let root =
            std::env::temp_dir().join(format!("zilobase-capture-recovery-{}", std::process::id()));
        let meeting = root.join("meeting-recordings").join("test-meeting");
        fs::create_dir_all(&meeting).unwrap();
        let external = root.join("unrelated.wav");
        ...
        let checkpoint = RecoverableMeetingCapture {
            ...
            elapsed_ms: 1,
            sample_rate: 24_000,
            audio_path: external.to_string_lossy().into_owned(),
        };
        fs::write(
            meeting.join("checkpoint.json"),
            serde_json::to_vec(&checkpoint).unwrap(),
        )
        .unwrap();
        let app = AppHandle::new(root.clone(), Arc::new(Mutex::new(io::stdout())));
        assert!(meeting_capture_recoverable_sessions(app)
            .unwrap()
            .is_empty());
