...
export function register({ assert, loadModule, test }) {
  ...
  test("replacement clears every server-scoped store and supports both deep-link launch modes", async () => {
    const replacement = await readFile(
      new URL("../src/lib/desktop-server-replacement.ts", import.meta.url),
      "utf8",
    );
    const controller = await readFile(
      new URL(
        "../src/components/desktop-server-replacement-controller.tsx",
        import.meta.url,
      ),
      "utf8",
    );
    const handler = await readFile(
      new URL(
        "../src/components/desktop-deep-link-handler.tsx",
        import.meta.url,
      ),
      "utf8",
    );
