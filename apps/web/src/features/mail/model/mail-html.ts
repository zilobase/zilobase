...
export function sanitizeMailHtml(
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="referrer" content="no-referrer"><meta http-equiv="Content-Security-Policy" content="default-src 'none'; img-src ${imageSources}; style-src 'unsafe-inline'; font-src data:"><style>html,body{height:auto!important;min-height:0!important;background:transparent!important}body{font:14px/1.6 system-ui,sans-serif;margin:0;color:CanvasText;overflow:hidden}img{max-width:100%;height:auto}img[data-zilobase-external-image],img[data-zilobase-inline-image=unavailable]{display:inline-block;min-width:10rem;min-height:1.5rem;border:1px dashed GrayText}</style></head><body>${sanitized}</body></html>`
}
