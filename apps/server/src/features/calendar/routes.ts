...
calendarRoutes.get("/configuration", c => { const ports = c.get("runtimePorts"); return c.json(inspectCalendarConfiguration(c.env, { background: Boolean(ports?.jobs), realtime: Boolean(ports?.fanout) })) });
