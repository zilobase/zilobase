...
[Browser authorization](../../../apps/desktop/electron/main/oauth.mjs) owns an ephemeral loopback listener, PKCE challenge, state and issuer checks, cancellation and token exchange. [Electron diagnostics](../../../apps/desktop/electron/main/diagnostics.mjs) filter renderer fields before logging and export bounded ZIPs; [updates](../../../apps/desktop/electron/main/updater.mjs) use the Electron Builder feed once packaging adds its platform metadata. [Package configuration](../../../apps/desktop/electron-builder.yml) registers the scheme, bundles the sidecar and defines macOS signing entitlements. The Tauri host remains the release target until live capture, signing and installer parity are verified.

[Meeting capture main IPC](../../../apps/desktop/electron/main/capture.mjs) supervises the sidecar process with bounded request size and timeouts, validates a start request before OS prompts, asks macOS for microphone access before input capture, and forwards only capture events to the preload. The [sidecar protocol](../../../apps/desktop/electron/sidecar/src/main.rs) invokes the bounded CPAL, transport and WAV recovery engine under [meetings](../../../apps/desktop/electron/sidecar/src/meetings/mod.rs). It writes to the same `meeting-recordings` location as Tauri. This engine copy is temporary until a shared native crate can serve both hosts; release requires live device, crash recovery and transport tests on each OS.
...
For Electron, build a local package and run the [isolated smoke test](../../../apps/desktop/e2e/electron-smoke.mjs). It exercises the packaged preload and main IPC, a profile snapshot round trip, encrypted credential round trip and diagnostic redaction without touching the release app's user data. The [experimental CI matrix](../../../.github/workflows/electron-desktop.yml) runs this on each OS; the [self-host flow](../../../apps/desktop/e2e/electron-selfhost.mjs) tests server selection when an instance is available. OAuth and device capture still require separate live acceptance tests.
