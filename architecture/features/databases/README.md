...
A database is page-backed; data sources, rows, views and property values are separate persisted concepts. Rows now have a nullable, backfilled `NUMERIC(30,10)` order key alongside the active integer-position compatibility field. [Shared order-key utilities](../../../packages/features/src/databases/order-key.ts) encode the decimal as a scaled bigint for deterministic cross-runtime midpoint calculation. Existing inserts and full-order operations take a transaction-scoped source advisory lock and dual-write order keys; precision exhaustion rebalances keys at intervals of 1024. Integer row positions and page-item placement positions remain the v1 compatibility projection. The [v2 persistence migration](../../../apps/server/drizzle/0090_database_mutation_journal.sql) also adds the durable versioned mutation journal and idempotent command receipts; command traffic now writes both tables while legacy producers continue using the v1 outbox path. Resource and data-source access checks constrain mutations. Formula and value rules also have shared implementations.
