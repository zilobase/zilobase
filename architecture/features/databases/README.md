...
Row/property changes can update realtime outboxes, automations and page navigation. The common [database commit helper](../../../apps/server/src/features/databases/core/commit.ts) gives internal writers a shared server-generated command ID and atomically stores one source-v3 journal event before its delivery-only outbox reference. Partial internal deltas become scoped reset events so consumers never ingest partial entities. After commit, the request publishes directly to the source room and removes the outbox row on success; the background drain retries failures and process interruptions. Missing history is recovered from the source feed rather than a host clone. Preserve mutation origin and transaction ordering. Source revisions and scoped cache reconciliation prevent stale UI after writes.
