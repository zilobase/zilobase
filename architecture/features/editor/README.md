...
[connection-session](../../../apps/web/src/features/editor/collaboration/connection-session.ts) owns deferred ticket resolution, provider attachment, awareness, synchronization confirmation and disposal. Its production services come from the offline connection owner and platform request runtime; controlled tests supply those mechanisms without sockets. React binds session events to state in `use-page-collaboration.ts`. Local document opening and debounced persistence remain in that hook and the offline feature. Connectivity changes reconnect the provider without reopening a downloaded document. Page composition supplies demo/local-only mode, keeping demo selection out of the reusable connection module.

A prepared ticket is consumed once. Provider creation applies ticket state first and disables immediate connection; the existing deferred React connect phase remains separate. A zero pending-update count confirms downloaded content only after provider synchronization. Access denial blocks the downloaded item; transient network failure leaves downloaded editing local. Disposal cancels scheduled startup, aborts ticket fetching and destroys the provider before clearing exposed state. Late ticket and provider events are ignored.
